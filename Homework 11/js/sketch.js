...
let toaster1Health = 100
let toaster2Health = 100
let toaster3Health = 100
let particles = []
let attackPaths = [];
...
function preload() {
    ...
    attackPaths = loadStrings("./images/idle/idle.txt");
}
    ...
    if (!gameOver && score < winScore && health > 0) {
        ...
        if (health <= 0) {
            ...
            if (keyIsDown(68)) {
                ...
                if (toasterImage1 != null) {
                    if (dist(myAnimation.getCurrentAnimation().position.x, myAnimation.getCurrentAnimation().position.y, toasterImage1.position.x, toasterImage1.position.y) < 200) {
                        createParticles(toasterImage1.position.x, toasterImage1.position.y);
                        toaster1Health -= 1;
                        if(toaster1Health <= 0)
                        {
                            toasterImage1.remove();
                            toasterImage1 = null;
                        }    
                    }
                }
                if (toasterImage2 != null) {
                    if (dist(myAnimation.getCurrentAnimation().position.x, myAnimation.getCurrentAnimation().position.y, toasterImage2.position.x, toasterImage2.position.y) < 200) {
                        createParticles(toasterImage2.position.x, toasterImage2.position.y);
                        toaster2Health -= 1;
                        if(toaster2Health <= 0)
                        {
                            toasterImage2.remove();
                            toasterImage2 = null;
                        }    
                    }
                }
                    if (toasterImage3 != null) {
                        if (dist(myAnimation.getCurrentAnimation().position.x, myAnimation.getCurrentAnimation().position.y, toasterImage3.position.x, toasterImage3.position.y) < 200) {
                            createParticles(toasterImage3.position.x, toasterImage3.position.y);
                            toaster3Health -= 1;
                            if(toaster3Health <= 0)
                            {
                                toasterImage3.remove();
                                toasterImage3 = null;
                            }    
                        }
                    }
